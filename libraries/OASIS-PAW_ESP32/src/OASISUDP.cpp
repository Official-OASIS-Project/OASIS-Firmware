...
uint8_t OASISUDP::begin(IPAddress address, uint16_t port){
  stop();

  server_port = port;

  tx_buffer = new char[13500];
  if(!tx_buffer){
...
uint8_t OASISUDP::begin(uint16_t p){
  return begin(IPAddress(INADDR_ANY), p);
}

uint8_t OASISUDP::beginMulticast(IPAddress a, uint16_t p){
...
int OASISUDP::beginMulticastPacket(){
...
int OASISUDP::beginPacket(IPAddress ip, uint16_t port){
  remote_ip = ip;
  remote_port = port;
  return beginPacket();
}

int OASISUDP::beginPacket(const char *host, uint16_t port){
...
size_t OASISUDP::write(uint8_t data){
  if(tx_buffer_len == 13500){
    endPacket();
    tx_buffer_len = 0;
  }
...
size_t OASISUDP::write(const uint8_t *buffer, size_t size){
  size_t i;
  for(i=0;i<size;i++)
    write(buffer[i]);
  return i;
...
int OASISUDP::read(unsigned char* buffer, size_t len){
  return read((char *)buffer, len);
}

int OASISUDP::read(char* buffer, size_t len){
  if(!rx_buffer) return 0;
...
IPAddress OASISUDP::remoteIP(){
  return remote_ip;
}

uint16_t OASISUDP::remotePort(){
  return remote_port;
}
